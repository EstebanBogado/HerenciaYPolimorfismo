...
public class Traslados {
	 ...
	 * @return
	 */
	public void costoBrutoDelViaje(Viaje viaje) {

		this.costoBrutoDelViaje = this.costoMinimoViaje + (viaje.getDestinoViaje().getDistancia() * this.costoKm);
		viaje.setCostoFinalViaje(costoBrutoDelViaje);
	...
	public boolean agregarViaje(Viaje viaje) {
		for (int i = 0; i < CANTIDAD_DE_VIAJES; i++) {
			if (viajes[i] == null) {
				...
				costoBrutoDelViaje(this.viajes[i]);
				viaje.setCostoFinalViaje(getCostoBrutoDelViaje());
				this.costoViajesAcumulados += viaje.getCostoViajeConDescuento();
				viaje.setNroDeViaje(i + 1);
